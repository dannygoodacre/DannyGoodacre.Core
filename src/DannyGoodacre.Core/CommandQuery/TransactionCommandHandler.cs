...
public abstract class TransactionCommandHandler<TCommandRequest>(ILogger logger, ITransactionalUnitOfWork transactionalUnitOfWork)
    : CommandHandler<TCommandRequest>(logger) where TCommandRequest : ICommand
{
    /// <summary>
    ...
    /// </returns>
    protected async override Task<Result> ExecuteAsync(TCommandRequest command, CancellationToken cancellationToken)
    {
